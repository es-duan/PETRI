...
pA <- anc_LFC +
  ...
  annotate("text",
           x = min_gamma + 0.4, y = max_psi - 0.05,
           label = "a", color = "white", size = 12) +
...
pB <- mut_LFC +
  ...
  guides(shape = "none",
         color = "none",
         fill = guide_colourbar(theme = theme(legend.text = element_text(size = 8))))
